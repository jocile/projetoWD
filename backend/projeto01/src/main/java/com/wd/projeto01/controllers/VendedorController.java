...
import com.wd.projeto01.service.VendedorService;
...
public class VendedorController {
  ...
  public ResponseEntity<List<VendedorDto>> listarVendedores() {
    List<VendedorDto> listaVendedores = servVendedor.findAll().stream().map(x -> new VendedorDto(x.getNome()))
  ...
  public ResponseEntity<Void> cadastrarVendedor(@RequestBody VendedorNewDTO vendedorNewDto) {
    ...
    URI uri = ServletUriComponentsBuilder.fromCurrentRequest().path("/{id}").buildAndExpand(vend.getId()).toUri();
